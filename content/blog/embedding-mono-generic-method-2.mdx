...
A while ago I wrote about <Link to="/blog/embedding-mono-generic-method-1">how to invoke a C# generic method</Link>, by using a helper method in the assembly. In this post, we will see how to invoke generic methods using solely the Mono embedding API. A much preferred alternative, since you don’t pollute your assemblies with helper methods that are used by C++ only.
