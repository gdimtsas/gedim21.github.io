...
Following up on a previous post that showed <Link to="/blog/embedding-mono-in-cpp">how to embed Mono in a C++ application</Link>, we will see how it is possible to debug C# code that runs on the host application, using Xamarin Studio (or MonoDevelop).
